...
/obj/machinery/computer/cloning/proc/findscanner()
	...
	if(!scannerf)
		...
		for(var/obj/machinery/dna_scannernew/S in get_area(A))
			return S
...
/obj/machinery/computer/cloning/proc/findcloner()
	var/num = 1
	var/area/A = get_area(src)
	for(var/obj/machinery/clonepod/P in get_area(A))
		if(!P.connected)
			pods += P
			P.connected = src
...
/obj/machinery/computer/cloning/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1)
	user.set_machine(src)
...
/obj/machinery/computer/cloning/Topic(href, href_list)
	...
	if ((href_list["scan"]) && (!isnull(scanner)))
		...
		addtimer(new Callback(src, .proc/scan_mob, scanner.occupant), 2 SECONDS)
