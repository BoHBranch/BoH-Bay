...
/obj/machinery/power/apc/updateDialog()
	if (stat & (MACHINE_BROKEN_HEALTH|MAINT))
		return
	..()
...
/obj/machinery/power/apc/on_update_icon()
	...
	if(update & 2)
		if(overlays.len)
			overlays.Cut()
		if(!(stat & (MACHINE_BROKEN_HEALTH|MAINT)) && update_state & UPDATE_ALLGOOD)
	...
	if(update & 3)
		...
		else if(!(stat & (MACHINE_BROKEN_HEALTH|MAINT)) && update_state & UPDATE_ALLGOOD)
			var/color
			switch(charging)
				if(0)
					color = "#f86060"
...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if(isCrowbar(W) && user.a_intent != I_HURT)//bypass when on harm intend to actually make use of the cover hammer off check further down.
		...
		if((stat & MACHINE_BROKEN_HEALTH) || (hacker && !hacker.hacked_apcs_hidden))
	...
	if (istype(W, /obj/item/card/id)||istype(W, /obj/item/modular_computer))
		...
		else if(stat & (MACHINE_BROKEN_HEALTH|MAINT))
	...
	if(istype(W, /obj/item/module/power_control))
		...
		if(do_after(user, 10, src) && has_electronics == 0 && opened && !(stat & MACHINE_BROKEN_HEALTH))
			has_electronics = 1
	...
	if(isWelder(W))
		...
		if(do_after(user, 50, src) && opened && has_electronics == 0 && !terminal())
			if(!WT.remove_fuel(3, user))
				return TRUE
			if (emagged || (stat & MACHINE_BROKEN_HEALTH) || opened==2)
	...
	if (istype(W, /obj/item/frame/apc))
		...
		if((stat & MACHINE_BROKEN_HEALTH) || (hacker && !hacker.hacked_apcs_hidden))
			...
			if(do_after(user, 50, src) && opened && !has_electronics && ((stat & MACHINE_BROKEN_HEALTH) || (hacker && !hacker.hacked_apcs_hidden)))
				user.visible_message(\
...
/obj/machinery/power/apc/Topic(href, href_list)
	...
	if (href_list["lock"])
		...
		if(istype(usr, /mob/living/silicon))
			if(emagged || (stat & (MACHINE_BROKEN_HEALTH|MAINT)))
...
/obj/machinery/power/apc/set_broken(new_state)
	if(!new_state || (stat & MACHINE_BROKEN_HEALTH))
		return ..()
...
/obj/machinery/power/apc/proc/set_light_color(var/color) //Mazian's alert lighting, ported by PurplePineapple
	var/lightmode
	switch(color)
		if("red")
