...
/obj/machinery/resleever/proc/bad_go_out(var/mob/living/carbon/human/occupant) //#VESTA.BAY Lets give some consequences for it
	if(occupant)
		if(resleeving)
			set waitfor = FALSE
			var/obj/item/organ/external/affected = occupant.get_organ(BP_HEAD)
			affected.take_external_damage(rand(20,40), rand(1, 10), DAM_SHARP)
