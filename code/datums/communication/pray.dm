...
/decl/communication_channel/pray/do_communicate(var/mob/communicator, var/message, var/speech_method_type)
	...
	for(var/m in GLOB.player_list)
		...
		if(M.client.holder && M.client.get_preference_value(/datum/client_preference/staff/show_chat_prayers) == GLOB.PREF_SHOW)
			receive_communication(communicator, M, "\[<A HREF='?_src_=holder;adminspawncookie=\ref[communicator]'>SC</a>\] \[<A HREF='?_src_=holder;narrateto=\ref[communicator]'>DN</a>\]<span class='notice'>[icon2html(src, viewers(src))]cross] <b><font color=purple>PRAY: </font>[key_name(communicator, 1)]: </b>[message]</span>")
		else if(communicator == M) //Give it to ourselves
			receive_communication(communicator, M, "<span class='notice'>[icon2html(src, viewers(src))]cross] <b>You send the prayer, \"[message]\" out into the heavens.</b></span>")
