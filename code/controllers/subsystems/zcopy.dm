...
/client/proc/analyze_openturf(turf/T)
	...
	for (var/thing in found_oo)
		...
		if (istype(A, /atom/movable/openspace/overlay))
			...
			out += "<li>[icon2html(src, viewers(src))]A] plane [A.plane], layer [A.layer], depth [OO.depth], associated Z-level [AA.z] - [OO.type] copying [AA] ([AA.type], eventually [OO.mimiced_type])</li>"
		else if (isturf(A))
			if (A == T)
				out += "<li>[icon2html(src, viewers(src))]A] plane [A.plane], layer [A.layer], Z-level [A.z] - [A] ([A.type]) - <font color='green'>SELF</font></li>"
			else	// foreign turfs - not visible here, but good for figuring out layering
				out += "<li>[icon2html(src, viewers(src))]A] plane [A.plane], layer [A.layer], Z-level [A.z] - [A] ([A.type]) - <font color='red'>FOREIGN</font></li>"
		else
			out += "<li>[icon2html(src, viewers(src))]A] plane [A.plane], layer [A.layer], Z-level [A.z] - [A] ([A.type])</li>"
