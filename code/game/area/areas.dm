...
/area/proc/play_ambience(var/mob/living/L)
	...
	if(hum)
		if(!L.client.ambience_playing)
			...
			L.playsound_simple(T,sound('sound/ambience/shipambience.ogg', repeat = 1, wait = 0, volume = 20, channel = GLOB.ambience_sound_channel))
	else
		if(L.client.ambience_playing)
	...
	if(L.lastarea != src)
		if(LAZYLEN(forced_ambience))
			...
			L.playsound_simple(T,sound(pick(forced_ambience), repeat = 1, wait = 0, volume = 25, channel = GLOB.lobby_sound_channel))
	...
	if(ambience.len && prob(5) && (world.time >= L.client.played + 3 MINUTES))
		L.playsound_simple(T, sound(pick(ambience), repeat = 0, wait = 0, volume = 15, channel = GLOB.lobby_sound_channel))
