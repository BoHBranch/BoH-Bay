...
/datum/game_mode/meteor/post_setup()
	...
	next_wave = round_duration_in_ticks_lag + meteor_grace_period
...
/datum/game_mode/meteor/proc/on_enter_field()
	...
	next_wave = round_duration_in_ticks_lag + meteor_wave_delay

/datum/game_mode/meteor/process()
	// Send an alert halfway through the round.
	if((round_duration_in_ticks_lag >= (next_wave / 2)) && !alert_sent)
		on_meteor_warn()
	// And then another one when the meteors start flying around.
	if((round_duration_in_ticks_lag >= next_wave) && (alert_sent == 1))
		on_enter_field()
	if((round_duration_in_ticks_lag >= METEOR_FAILSAFE_THRESHOLD) && (meteor_severity < 15) && !failsafe_triggered)
		log_and_message_admins("Meteor mode severity failsafe triggered: Severity forced to 15.")
		meteor_severity = 15
		failsafe_triggered = 1

	if(round_duration_in_ticks_lag >= next_wave)
