...
/obj/structure/deity/pylon/hear_talk(mob/M as mob, text, verb, datum/language/speaking)
	...
	to_chat(linked_god, "[icon2html(src, viewers(src))]src] <span class='game say'><span class='name'>[M]</span> (<A href='?src=\ref[linked_god];jump=\ref[src];'>P</A>) [verb], [linked_god.pylon == src ? "<b>" : ""]<span class='message'><span class='body'>\"[text]\"</span></span>[linked_god.pylon == src ? "</b>" : ""]</span>")
	if(linked_god.minions.len)
		for(var/minion in linked_god.minions)
			...
			if(mind.current && mind.current.eyeobj) //If it is currently having a vision of some sort
				to_chat(mind.current,"[icon2html(src, viewers(src))]src] <span class='game say'><span class='name'>[M]</span> (<A href='?src=\ref[src];vision_jump=\ref[src];'>J</A>) [verb], <span class='message'<span class='body'>\"[text]\"</span></span>")