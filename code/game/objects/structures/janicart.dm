...
/obj/structure/janitorialcart/examine(mob/user, distance)
	. = ..()
	if(distance <= 1)
		to_chat(user, "[src] [icon2html(src, viewers(src))]src] contains [reagents.total_volume] unit\s of liquid!")
...
/obj/structure/bed/chair/janicart/examine(mob/user, distance)
	...
	to_chat(user, "[icon2html(src, viewers(src))]src] This [callme] contains [reagents.total_volume] unit\s of water!")
	if(mybag)
