...
/obj/item/weapon/tank/proc/check_status()
	...
	if(pressure > TANK_FRAGMENT_PRESSURE)
		...
		if(integrity <= 0)
			...
			visible_message("[icon2html(src, viewers(src))]src] <span class='danger'>\The [src] flies apart!</span>", "<span class='warning'>You hear a bang!</span>")
			T.hotspot_expose(air_contents.temperature, 70, 1)
		...
		if((integrity <= 19 || leaking) && !valve_welded)
			...
			if(!leaking)
				visible_message("[icon2html(src, viewers(src))]src] <span class='warning'>\The [src] relief valve flips open with a hiss!</span>", "You hear hissing.")
