		...
		for(var/mob/M in GLOB.player_list)
			if(M.z == epicenter.z)
				...
				if(dist <= round(max_range + world.view - 2, 1))
					M.playsound_simple(epicenter, get_sfx("explosion"), 100, 1, frequency, falloff = 5) // get_sfx() is so that everyone gets the same sound
				else if(dist <= far_dist)
					var/far_volume = Clamp(far_dist, 30, 50) // Volume is based on explosion size and dist
					far_volume += (dist <= far_dist * 0.5 ? 50 : 0) // add 50 volume if the mob is pretty close to the explosion
					M.playsound_simple(epicenter, 'sound/effects/explosionfar.ogg', far_volume, 1, frequency, falloff = 5)
