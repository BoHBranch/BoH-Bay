...
/obj/machinery/body_scanconsole/OnTopic(mob/user, href_list)
	if(href_list["scan"])
		if (!connected.occupant)
			to_chat(user, "[icon2html(src, viewers(src))]<span class='warning'>The body scanner is empty.</span>")
			return TOPIC_REFRESH
		if (!istype(connected.occupant))
			to_chat(user, "[icon2html(src, viewers(src))]<span class='warning'>The body scanner cannot scan that lifeform.</span>")
			return TOPIC_REFRESH
	...
	if (href_list["print"])
		if (!data["scan"])
			to_chat(user, "[icon2html(src, viewers(src))]<span class='warning'>Error: No scan stored.</span>")
			return TOPIC_REFRESH
	...
	if(href_list["push"])
		if(!connected_displays.len && !FindDisplays())
			to_chat(user, "[icon2html(src, viewers(src))]<span class='warning'>Error: No configured displays detected.</span>")
			return TOPIC_REFRESH
