...
/obj/machinery/botany/proc/finished_task()
	...
	if(failed_task)
		failed_task = 0
		visible_message("[icon2html(src, viewers(src))]src] [src] pings unhappily, flashing a red warning light.")
	else
		visible_message("[icon2html(src, viewers(src))]src] [src] pings happily.")

	if(eject_disk)
		eject_disk = 0
		if(loaded_disk)
			loaded_disk.dropInto(loc)
			visible_message("[icon2html(src, viewers(src))]src] [src] beeps and spits out [loaded_disk].")
			loaded_disk = null
...
/obj/machinery/botany/Topic(href, href_list)
	...
	if(href_list["eject_packet"])
		...
		seed.update_seed()
		visible_message("[icon2html(src, viewers(src))]src] [src] beeps and spits out [seed].")
