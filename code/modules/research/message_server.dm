	...
	for (var/obj/machinery/requests_console/Console in allConsoles)
		...
		if (ckey(Console.department) == ckey(recipient))
			...
			if(priority > 1)
				...
				Console.audible_message("[icon2html(src, viewers(src))]<span class='warning'>\The [Console] announces: 'High priority message received from [sender]!'</span>", hearing_distance = 8)
				...
				if(!Console.silent)
					...
					Console.audible_message("[icon2html(src, viewers(src))]<span class='notice'>\The [Console] announces: 'Message received from [sender].'</span>", hearing_distance = 5)
