/////////
// Just the GAS' grab with some minor changes.
/////////
/obj/item/grab/armalis

	type_name = GRAB_ARMALIS
	start_grab_name = ARMALIS_PASSIVE

/obj/item/grab/armalis/init()
	if(!(. = ..()))
		return
...
/datum/grab/armalis/on_hit_grab(var/obj/item/grab/G)
	var/mob/living/carbon/human/affecting = G.affecting
	var/mob/living/carbon/human/assailant = G.assailant

	var/crush_damage = rand(12,24)
...
/datum/grab/armalis/on_hit_harm(var/obj/item/grab/G)
	var/mob/living/carbon/human/affecting = G.affecting
	var/mob/living/carbon/human/assailant = G.assailant
...
// This causes the assailant to crush the affecting mob.
/datum/grab/armalis/proc/crush(var/obj/item/grab/G, var/attack_damage)
	...
	if(prob(30))
		...
		G.affecting.apply_damage(hit_damage, BRUTE, hit_zone, DAM_SHARP, used_weapon = "claw punctures")
		...
		G.affecting.visible_message("<span class='danger'>[G.assailant]'s talons dig in painfully!</span>")
	else
		G.affecting.apply_damage(attack_damage, BRUTE, hit_zone, used_weapon = "crushing")
	playsound(get_turf(G.assailant), 'sound/weapons/heavysmash.ogg', 25, 1, -1)
...
/datum/grab/armalis/proc/masticate(var/obj/item/grab/G, var/attack_damage)
	var/hit_zone = G.assailant.zone_sel.selecting
	var/obj/item/organ/external/damaging = G.affecting.get_organ(hit_zone)

	G.affecting.apply_damage(attack_damage, BRUTE, hit_zone, DAM_SHARP|DAM_EDGE, used_weapon = "vise")
