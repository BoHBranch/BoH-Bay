...
/obj/item/weapon/slimemutator
	name = "slime mutation agent"
	desc = "A potent chemical mix that will cause a slime to be 12% more likely to mutate."
	icon = 'icons/obj/chemical.dmi'
	icon_state = "bottle3"

/obj/item/weapon/slimemutator/attack(mob/living/L, mob/user)
	if(!istype(L, /mob/living/carbon/slime))//If target is not a slime.
		to_chat(user, "<span class='warning'> The steroid only works on baby slimes!</span>")
		return ..()
	var/mob/living/carbon/slime/M = L
	if(M.stat)
		to_chat(user, "<span class='warning'> The slime is dead!</span>")
		return..()
	to_chat(user, "You feed the slime the steroid. It now is more likely to mutate.")
	M.mutation_chance = min(M.mutation_chance + 12, 100)
	qdel(src)

/obj/item/weapon/slimestabilizer
	name = "slime stabilization agent"
	desc = "A potent chemical mix that will cause a slime to be 12% less likely to mutate."
	icon = 'icons/obj/chemical.dmi'
	icon_state = "bottle17"

/obj/item/weapon/slimestabilizer/attack(mob/living/L, mob/user)
	if(!istype(L, /mob/living/carbon/slime))//If target is not a slime.
		to_chat(user, "<span class='warning'> The steroid only works on baby slimes!</span>")
		return ..()
	var/mob/living/carbon/slime/M = L
	if(M.stat)
		...
		return..()
	to_chat(user, "You feed the slime the steroid. It now is more likely to mutate.")
	M.mutation_chance = max(M.mutation_chance - 12, 0)
	qdel(src)
