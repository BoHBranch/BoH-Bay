...
/mob/living/carbon/alien/diona/examine(mob/user)
	. = ..()
	if(holding_item)
		to_chat(user, "<span class='notice'>It is holding [icon2html(src, viewers(src))] \a [holding_item].</span>")
	if(hat)
		to_chat(user, "<span class='notice'>It is wearing [icon2html(src, viewers(src))] \a [hat].</span>")
