...
/mob/living/simple_animal/hostile/retaliate/giant_spider
	name = "giant spider"
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/can_do_maneuver(var/decl/maneuver/maneuver, var/silent = FALSE)
	. = ..() && can_act()

//guards - less venomous, tanky, slower, prioritises protecting nurses
/mob/living/simple_animal/hostile/retaliate/giant_spider/guard
	...
	var/vengance
	var/berserking
	var/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/paired_nurse

//nursemaids - these create webs and eggs - the weakest and least threatening
/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse
	...
	var/max_eggs = 8
	var/infest_chance = 8
	var/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/paired_guard

	//things we can't encase in a cocoon
	var/list/cocoon_blacklist = list(/mob/living/simple_animal/hostile/retaliate/giant_spider,
									 /obj/structure/closet)

//hunters - the most damage, fast, average health and the only caste tenacious enough to break out of nets
/mob/living/simple_animal/hostile/retaliate/giant_spider/hunter
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/spitter
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/spitter/molly
	name = "Molly"
	desc = "This is Molly, the Radio Host's pet spider!"

//General spider procs
/mob/living/simple_animal/hostile/retaliate/giant_spider/Initialize(var/mapload, var/atom/parent)
	get_light_and_color(parent)
	spider_randomify()
	update_icon()
	. = ..()

/mob/living/simple_animal/hostile/retaliate/giant_spider/proc/spider_randomify() //random math nonsense to get their damage, health and venomness values
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/on_update_icon()
	if(stat == DEAD)
		overlays.Cut()
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/FindTarget()
	. = ..()
	if(.)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/AttackingTarget()
	. = ..()
	if(isliving(.))
		if(health < maxHealth)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/Life()
	. = ..()
	if(!.)
		return FALSE
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/proc/disable_stop_automated_movement()
	stop_automated_movement = 0
	walk(src,0)
	kick_stance()

/mob/living/simple_animal/hostile/retaliate/giant_spider/proc/divorce()
	return

/****************
Guard caste procs
****************/
/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/Life()
	. = ..()
	if(!.)
		return FALSE
	if(berserking)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/death()
	. = ..()
	divorce()

/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/Destroy()
	. = ..()
	divorce()

/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/divorce()
	if(paired_nurse)
		if(paired_nurse.paired_guard)
			paired_nurse.paired_guard = null
		paired_nurse = null

/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/proc/find_nurse()
	for(var/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/N in ListTargets(10))
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/proc/protect(mob/nurse)
	stop_automated_movement = 1
	walk_to(src, nurse, 2, move_to_delay)
	addtimer(CALLBACK(src, .proc/disable_stop_automated_movement), 5 SECONDS)

/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/proc/go_berserk()
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/guard/proc/calm_down()
	berserking = FALSE
...
Nurse caste procs
****************/
/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/divorce()
	if(paired_guard)
		if(paired_guard.paired_nurse)
			paired_guard.paired_nurse = null
	paired_guard = null

/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/death()
	. = ..()
	if(paired_guard)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/Destroy()
	. = ..()
	divorce()

/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/AttackingTarget()
	. = ..()
	if(ishuman(.))
		var/mob/living/carbon/human/H = .
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/proc/GiveUp(var/C)
	spawn(100)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/nurse/Life()
	. = ..()
	if(!.)
		return FALSE
	if(stance == HOSTILE_STANCE_IDLE)
...
Hunter caste procs
*****************/
/mob/living/simple_animal/hostile/retaliate/giant_spider/hunter/MoveToTarget()
	if(!can_act() || perform_maneuver(/decl/maneuver/leap/spider, target_mob))
		return
	..()

/mob/living/simple_animal/hostile/retaliate/giant_spider/hunter/get_jump_distance()
	return leap_range

/mob/living/simple_animal/hostile/retaliate/giant_spider/hunter/perform_maneuver(var/maneuver, var/atom/target)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/hunter/throw_impact(atom/hit_atom)
...
Spitter caste procs
******************/
/mob/living/simple_animal/hostile/retaliate/giant_spider/spitter/Life()
	. = ..()
	if(!.)
		return FALSE
	if(venom_charge <= 0)
...
/mob/living/simple_animal/hostile/retaliate/giant_spider/spitter/Shoot()
	..()
	venom_charge--
