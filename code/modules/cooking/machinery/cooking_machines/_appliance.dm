...
/obj/machinery/appliance/proc/attempt_toggle_power(mob/user)
	...
	stat ^= POWER_USE_ACTIVE // Toggles power
	use_power = !(stat & POWER_USE_ACTIVE) && POWER_USE_ACTIVE // If on, use active power, else use no power
	if(user)
...
/obj/machinery/appliance/proc/can_insert(obj/item/I, mob/user)
	...
	if(istype(G))
		...
		if(!can_cook_mobs)
			to_chat(user, "<span class='warning'>That's not going to fit.")
			return 0

		if(!isliving(G.affecting))
			to_chat(user, "<span class='warning'>You can't cook that.")
			return 0
	...
	if (!has_space(I))
		to_chat(user, "<span class='warning'>There's no room in [src] for that!")
		return 0
...
/obj/machinery/appliance/attackby(obj/item/I, mob/user)
	...
	if(!cook_type || (stat & (MACHINE_BROKEN_HEALTH)))
		to_chat(user, "<span class='warning'>\The [src] is not working.")
		return
...
/obj/machinery/appliance/proc/burn_food(datum/cooking_item/CI)
	...
	// Produce nasty smoke.
	visible_message("<span class='danger'>\The [src] vomits a gout of rancid smoke!")
