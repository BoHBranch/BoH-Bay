...
/obj/machinery/atm/emag_act(var/remaining_charges, var/mob/user)
	if(!emagged)
		...
		to_chat(user, "[icon2html(src, viewers(src))]src] <span class='warning'>[src] beeps: \"[response]\"</span>")
		return 1
...
/obj/machinery/atm/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/weapon/card))
		if(emagged > 0)
			...
			to_chat(user, "[icon2html(src, viewers(src))]src] <span class='warning'>CARD READER ERROR. This system has been compromised!</span>")
			return
		if(stat & NOPOWER)
...
/obj/machinery/atm/interact(mob/user)
	...
	if(istype(user, /mob/living/silicon))
		to_chat(user, "[icon2html(src, viewers(src))]src] <span class='warning'>Artificial unit recognized. Artificial units do not currently receive monetary compensation, as per system banking regulation #1005.</span>")
		return
		...
		switch(href_list["choice"])
				...
				if(authenticated_account)
					...
					if(transfer_amount <= 0)
						...
						if(target_account && authenticated_account.transfer(target_account, transfer_amount, transfer_purpose))
							to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='info'>Funds transfer successful.</span>")
						else
							to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Funds transfer failed.</span>")

					else
						to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>You don't have enough funds to do that!</span>")
			if("view_screen")
					...
					if(!authenticated_account)
						...
						if(previous_account_number == tried_account_num && tried_pin)
							if(number_incorrect_tries >= max_pin_attempts)
								...
								to_chat(usr, "[icon2html(src, viewers(src))]src] <span class='warning'>Incorrect pin/account combination entered, [max_pin_attempts - number_incorrect_tries] attempts remaining.</span>")
								previous_account_number = tried_account_num
								playsound(src, 'sound/machines/buzz-sigh.ogg', 50, 1)
						else
							to_chat(usr, "[icon2html(src, viewers(src))]src] <span class='warning'>Unable to log in to account, additional information may be required.</span>")
							number_incorrect_tries = 0
					else
						...
						to_chat(usr, "[icon2html(src, viewers(src))]src] <span class='info'>Access granted. Welcome user '[authenticated_account.owner_name].'</span>")
			...
			if("insert_card")
				if(!held_card)
					...
					if(emagged > 0)
						to_chat(usr, "[icon2html(src, viewers(src))]src] <span class='warning'>The ATM card reader rejected your ID because this machine has been sabotaged!</span>")
					else
