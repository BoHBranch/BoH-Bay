...
/obj/item/device/eftpos/attackby(obj/item/O as obj, user as mob)
	...
	if(I)
		if(linked_account)
			...
			to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Unable to connect to linked account.</span>")
			...
			if(transaction_locked && !transaction_paid)
				if(transaction_amount <= E.worth)
					...
					if(linked_account.deposit(transaction_amount, purpose, machine_id))
						...
						src.visible_message("[icon2html(src, viewers(src))]src] \The [src] chimes.")
						transaction_paid = 1
					else
						to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Transaction failed! Please try again.</span>")
				else
					to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>\The [O] doesn't have that much money!</span>")
		else
			to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>EFTPOS is not connected to an account.</span>")
		...
		switch(href_list["choice"])
			if("change_code")
				...
				if(attempt_code == access_code)
					...
					to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Incorrect code entered.</span>")
			if("change_id")
			...
			if("link_account")
				...
				if(linked_account)
					if(linked_account.suspended)
						linked_account = null
						to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Account has been suspended.</span>")
				else
					to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Account not found.</span>")
			if("trans_purpose")
			...
			if("toggle_lock")
				if(transaction_locked)
					...
					to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>No account connected to send transactions to.</span>")
			if("scan_card")
				if(linked_account)
					...
					to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Unable to link accounts.</span>")
			if("reset")
				...
				if (istype(I, /obj/item/weapon/card))
					...
					if((access_cent_captain in C.access) || (access_hop in C.access) || (access_captain in C.access))
						access_code = 0
						to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='info'>Access code reset to 0.</span>")
			...
			if(linked_account)
				if(!linked_account.suspended)
					...
					if(D)
						...
						to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Unable to access account. Check security settings and try again.</span>")
				else
					to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>Connected account has been suspended.</span>")
			else
				to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='warning'>EFTPOS is not connected to an account.</span>")
	else if (istype(I, /obj/item/weapon/card/emag))
		if(transaction_locked)
			if(transaction_paid)
				to_chat(usr, "[icon2html(src, viewers(src))]src]<span class='info'>You stealthily swipe \the [I] through \the [src].</span>")
				transaction_locked = 0
				transaction_paid = 0
			else
				usr.visible_message("<span class='info'>\The [usr] swipes a card through \the [src].</span>")
