...
/obj/item/device/assembly/voice/hear_talk(mob/living/M as mob, msg)
	if(listening)
		...
		T.visible_message("[icon2html(src, viewers(src))]src] beeps, \"Activation message is '[recorded]'.\"")
	else
...
/obj/item/device/assembly/voice/activate()
	if(secured)
		if(!holder)
			...
			T.visible_message("[icon2html(src, viewers(src))]src] beeps, \"[listening ? "Now" : "No longer"] recording input.\"")
