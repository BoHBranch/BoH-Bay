	...
	if(activated)
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			...
			if(world.time - last_activation > interval)
				if(inserted_battery.battery_effect.effect == EFFECT_TOUCH)
					if(interval > 0)
						...
						if(holder)
							to_chat(holder, "The [icon2html(src, viewers(src))]src] [src] held by [holder] shudders in your grasp.")
						else
							src.loc.visible_message("The [icon2html(src, viewers(src))]src] [src] shudders.")
			...
			if(inserted_battery.stored_charge <= 0)
				src.loc.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src] buzzes.</span>", "<span class='notice'>[icon2html(src, viewers(src))]src] You hear something buzz.</span>")
				shutdown_emission()
			else if(world.time > time_end)
				src.loc.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src] chimes.</span>", "<span class='notice'>[icon2html(src, viewers(src))]src] You hear something chime.</span>")
				shutdown_emission()
		else
			src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src] buzzes.</span>", "<span class='notice'>[icon2html(src, viewers(src))]src] You hear something buzz.</span>")
			shutdown_emission()
...
/obj/item/weapon/anodevice/OnTopic(user, href_list)
	if(href_list["changetime"])
		...
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			activated = 1
			last_process = world.time
			src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src] whirrs.</span>", "<span class='notice'>[icon2html(src, viewers(src))]src] You hear something whirr.</span>")
			if(!inserted_battery.battery_effect.activated)
				inserted_battery.battery_effect.ToggleActivate(1)
