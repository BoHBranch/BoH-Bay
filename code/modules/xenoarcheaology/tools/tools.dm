...
/obj/item/device/gps/attack_self(var/mob/user as mob)
	to_chat(user, "<span class='notice'>[icon2html(src, viewers(src))] \The [src] flashes <i>[get_coordinates()]</i>.</span>")
...
/obj/item/device/depth_scanner/proc/scan_atom(var/mob/user, var/atom/A)
	...
	if(istype(A, /turf/simulated/mineral))
		...
		if((M.finds && M.finds.len) || M.artifact_find)
			...
			to_chat(user, "<span class='notice'>[icon2html(src, viewers(src))] [src] pings.</span>")
		...
		if(B.artifact_find)
			...
			to_chat(user, "<span class='notice'>[icon2html(src, viewers(src))] [src] pings [pick("madly","wildly","excitedly","crazily")]!</span>")
