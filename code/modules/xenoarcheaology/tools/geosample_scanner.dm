	...
	if(scanning)
		if(!scanned_item || scanned_item.loc != src)
			...
			if(scanner_seal_integrity <= 0 || (scanner_temperature >= 1273 && !rad_shield))
				stop_scanning()
				src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] buzzes unhappily. It has failed mid-scan!</span>", 2)

			if(prob(5))
				src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [pick("whirrs","chuffs","clicks")][pick(" excitedly"," energetically"," busily")].</span>", 2)
	else
		//gradually cool down over time
		if(scanner_temperature > 0)
			scanner_temperature = max(scanner_temperature - 5 - 10 * rand(), 0)
		if(prob(0.75))
			src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [pick("plinks","hisses")][pick(" quietly"," softly"," sadly"," plaintively")].</span>", 2)
...
/obj/machinery/radiocarbon_spectrometer/proc/complete_scan()
	src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] makes an insistent chime.</span>", 2)
