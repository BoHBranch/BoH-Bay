...
/obj/machinery/replicator/New()
	...
	fail_message = "<span class='notice'>[icon2html(src, viewers(src))]src] a [pick("loud","soft","sinister","eery","triumphant","depressing","cheerful","angry")] \
...
/obj/machinery/replicator/Process()
	if(spawning_types.len && !(stat & NOPOWER))
		...
		if(spawn_progress_time > max_spawn_time)
			src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src] pings!</span>")
			...
			src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src] [pick("clicks","whizzes","whirrs","whooshes","clanks","clongs","clonks","bangs")].</span>")
		...
		if(index > 0 && index <= construction.len)
			if(stored_materials.len > spawning_types.len)
				if(spawning_types.len)
					src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] a [pick("light","dial","display","meter","pad")] on [src]'s front [pick("blinks","flashes")] [pick("red","yellow","blue","orange","purple","green","white")].</span>")
				else
					src.visible_message("<span class='notice'>[icon2html(src, viewers(src))]src] [src]'s front compartment slides shut.</span>")
