...
/obj/item/weapon/vampiric/Process()
	...
	if(charges >= 0.1)
		if(prob(5))
			src.visible_message("<span class='warning'>[icon2html(src, viewers(src))]src] [src]'s eyes glow ruby red for a moment!</span>")
			charges -= 0.1
...
/obj/item/weapon/vampiric/hear_talk(mob/M as mob, text)
	..()
	if(world.time - last_bloodcall >= bloodcall_interval && (M in viewers(7, src)))
		bloodcall(M)
