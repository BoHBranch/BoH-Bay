...
/proc/AddBan(ckey, computerid, reason, bannedby, temp, minutes, address)
	...
	if ( Banlist.dir.Find("[ckey][computerid]") )
		...
		to_save_key(Banlist, "key", ckey)
		to_save_key(Banlist, "id", computerid)
		to_save_key(Banlist, "ip", address)
		to_save_key(Banlist, "reason", reason)
		to_save_key(Banlist, "bannedby", bannedby)
		to_save_key(Banlist, "temp", temp)
		if (temp)
			to_save_key(Banlist, "minutes", bantimestamp)
	return 1
...
/proc/RemoveBan(foldername)
	...
	from_save_key(Banlist, "key", key)
	from_save_key(Banlist, "id", id)
	Banlist.cd = "/base"
...
/proc/CreateBans()
	...
	for(i=0, i<1001, i++)
		...
		if(b)
			...
			to_save_key(Banlist, "key", "trash[i]")
		else
			Banlist.cd = "/base"
			Banlist.dir.Add("[last]trashid[i]")
			Banlist.cd = "/base/[last]trashid[i]"
			to_save_key(Banlist, "key", last)
		to_save_key(Banlist, "id", "trashid[i]")
		to_save_key(Banlist, "reason", "Trashban[i].")
		to_save_key(Banlist, "temp", a)
		to_save_key(Banlist, "minutes", CMinutes + rand(1,2000))
		to_save_key(Banlist, "bannedby", "trashmin")
		last = "trash[i]"
