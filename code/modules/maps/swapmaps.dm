...
swapmap
	...
	Write(savefile/S)
		...
		locked=1
		to_save_key(S, "id", id)
		to_save_key(S, "z", z2-z1+1)
		to_save_key(S, "y", y2-y1+1)
		to_save_key(S, "x", x2-x1+1)
		to_save_key(S, "areas", areas)
		...
		for(z=z1,z<=z2,++z)
			...
			for(y=y1,y<=y2,++y)
				...
				for(x=x1,x<=x2,++x)
					...
					to_save_key(S, "type", T.type)
					if(T.loc!=defarea) to_save_key(S, "AREA", areas[T.loc])
					T.Write(S)
					S.cd=".."
				S.cd=".."
			sleep()
		...
		for(z=z1,z<=z2,++z)
			...
			for(y=y1,y<=y2,++y)
				...
				for(x=x1,x<=x2,++x)
					S.cd="[x-x1+1]"
					var/tp
					from_save_key(S, "type", tp)
					var/turf/T=locate(x,y,z)
					T.loc.contents-=T
					T=new tp(locate(x,y,z))
					if("AREA" in S.dir)
						from_save_key(S, "AREA", n)
						var/area/A=areas[n]
						A.contents+=T
...
atom
	Write(savefile/S)
		for(var/V in vars-"x"-"y"-"z"-"contents"-"icon"-"overlays"-"underlays")
			if(issaved(vars[V]))
				if(vars[V]!=initial(vars[V])) from_save_key(S, V, vars[V])
				else S.dir.Remove(V)
		if(icon!=initial(icon))
			if(swapmaps_iconcache && swapmaps_iconcache[icon])
				to_save_key(S, "icon", swapmaps_iconcache[icon])
			else to_save_key(S, "icon", icon)
		// do not save mobs with keys; do save other mobs
		var/mob/M
		for(M in src) if(M.key) break
		if(overlays.len) to_save_key(S, "overlays", overlays)
		if(underlays.len) to_save_key(S, "underlays", underlays)
		if(contents.len && !isarea(src))
			var/list/l=contents
			if(M)
				l=l.Copy()
				for(M in src) if(M.key) l-=M
			if(l.len) to_save_key(S, "contents", l)
