...
/proc/DisplayTimeText(time_value, round_seconds_to = 0.1)
	var/second = FloorToY(time_value * 0.1, round_seconds_to)
	if(!second)
		return "right now"
	if(second < 60)
		return "[second] second[(second != 1)? "s":""]"
	var/minute = FloorToY(second / 60, 1)
	second = FloorToY(ModulusXY(second, 60), round_seconds_to)
	var/secondT
	if(second)
		secondT = " and [second] second[(second != 1)? "s":""]"
	if(minute < 60)
		return "[minute] minute[(minute != 1)? "s":""][secondT]"
	var/hour = FloorToY(minute / 60, 1)
	minute = ModulusXY(minute, 60)
	var/minuteT
	if(minute)
		minuteT = " and [minute] minute[(minute != 1)? "s":""]"
	if(hour < 24)
		return "[hour] hour[(hour != 1)? "s":""][minuteT][secondT]"
	var/day = FloorToY(hour / 24, 1)
	hour = ModulusXY(hour, 24)
	var/hourT
	if(hour)
