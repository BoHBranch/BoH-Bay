...
function run_byond_tests {
    ...
    run_test_ci "check globals build" "python3 tools/GenerateGlobalVarAccess/gen_globals.py bastionofhestia.dme code/_helpers/global_access.dm"
    run_test "check globals unchanged" "md5sum -c - <<< 'baf6134534402ae5e1b1876cd23fa604 *code/_helpers/global_access.dm'"
    run_test "build map unit tests" "scripts/dm.sh -DUNIT_TEST -M$MAP_PATH bastionofhestia.dme"
    run_test "check no warnings in build" "grep ', 0 warnings' build_log.txt"
    run_test "run unit tests" "DreamDaemon bastionofhestia.dmb -invisible -trusted -core 2>&1 | tee log.txt"
