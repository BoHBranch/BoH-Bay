/datum/extension/interactive/multitool/cryo/get_interact_window(obj/item/device/multitool/M, mob/user)
	. += buffer(M)
	. += "<HR><b>Connected Cloning Pods:</b><br>"
	var/obj/machinery/computer/cloning/C = holder
	for(var/atom/cloning_pod in C.pods)
		. += "[cloning_pod.name]<br>"
/*	//The clonning machinery are actually auto-seeking for clonepods and scanners. @r4iser
